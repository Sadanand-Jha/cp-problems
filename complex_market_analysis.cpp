...
void solve(vi &primes) {
    ...
    for(int i = 0; i < e; i++) {
        ...
        for(int j = 0; j * e + i < n; j++) {
            distribution.push_back(vec[j * e + i]);
        }
        ...
        for(int k = 0; k < distribution.size(); k++) {
            if(distribution[k] == 1) right++;
            else{
                // check prime 
                int x = lower_bound(primes.begin(), primes.end(), distribution[k]) - primes.begin();
                if(prime_idx != -1)count += left * (right + 1) + right;
                if(distribution[k] == primes[x]) {
                    prime_idx = i;
                    left = right;
                    right = 0;
                }
